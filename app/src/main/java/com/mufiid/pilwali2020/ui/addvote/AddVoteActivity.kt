...
class AddVoteActivity : AppCompatActivity(), IPaslonView, ITpsView, View.OnClickListener {
    ...
    private val dataPaslon = mutableListOf<Paslon>()
    ...
    private fun doUpload() {
        ...
        for (i in 0 until (PaslonAdapter.dataPaslon?.size ?: 0)) {
            val paslon = PaslonAdapter.dataPaslon?.get(i)
            if (paslon?.jumlah_suara_di_tps.isNullOrEmpty()) {
                // change data
                dataPaslon.add(Paslon(paslon?.nmPeserta, paslon?.foto, paslon?.id, paslon?.noPeserta, paslon?.jumlah_suara, ""))
            }
            rv_paslon.apply {
                setHasFixedSize(true)
                layoutManager = viewManager
                adapter = PaslonAdapter(dataPaslon)
            }
